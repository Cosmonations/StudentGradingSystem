...
int main(){
    ...
    recordFile.open("student_records.csv", ios::out); 
    
    if (recordFile.is_open()) {
        recordFile << "FullName" << " SubjectNumber" << " TermGradeAverage" << " FinalTermGrade" << endl;
        recordFile << studentName << " " << subjectNum << " " << termMeanAverage << " " << finalLetterGrade << endl;
        recordFile.close();
        cout << "Success! Student data successfully written to student_records.csv" << endl;
    } else {
        cout << "[ERROR] Could not open student_records.csv for writing." << endl;
    }
